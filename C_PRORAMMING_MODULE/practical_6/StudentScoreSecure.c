/* StudentScoresSecure.c
   ...
   Secure array operations with comprehensive input validation and bounds checking
   Aayush K.C. */
#include <stdio.h>
#include <stdbool.h>
#include <limits.h>
...
/* Validate array pointer is not NULL and size is within safe limits */
bool validateArrayParams(int scores[], int size) {
    if (scores == NULL) {
...
/* Read a single score safely: checks scanf success and score range */
bool readSingleScore(int *score) {
    if (score == NULL) return false;

    // Check scanf return value — if not 1, input was not a valid integer
    if (scanf("%d", score) != 1) {
        // Clear the bad input from the buffer to prevent infinite loop
        while (getchar() != '\n');
        return false;
    }
...
/* Read all scores with retry logic — up to MAX_ATTEMPTS per score */
bool readScores(int scores[], int size) {
    ...
    for (int i = 0; i < size; i++) {
        // Extra bounds check (defensive — should never trigger with correct loop)
        if (i >= size) {
        ...
        if (!validInput) {
            printf("Maximum attempts exceeded for score %d. Terminating.\n", i + 1);
            return false;
        }
    }
...
/* Find maximum score with bounds checking */
int findMaximum(int scores[], int size) {
    if (!validateArrayParams(scores, size)) return -1;
    int max = scores[0];
    for (int i = 1; i < size; i++) {
        if (i >= size) { printf("Error: Bounds exceeded in findMaximum.\n"); return -1; }
        if (scores[i] > max) max = scores[i];
    }
    return max;
}

/* Find minimum score with bounds checking */
int findMinimum(int scores[], int size) {
    if (!validateArrayParams(scores, size)) return -1;
    int min = scores[0];
    for (int i = 1; i < size; i++) {
        if (i >= size) { printf("Error: Bounds exceeded in findMinimum.\n"); return -1; }
        if (scores[i] < min) min = scores[i];
    }
    return min;
}

/* Display all scores safely */
void displayScores(int scores[], int size) {
    if (!validateArrayParams(scores, size)) return;
    printf("\nIndividual Scores:\n");
    for (int i = 0; i < size; i++) {
        if (i >= size) { printf("Error: Bounds exceeded in displayScores.\n"); return; }
        printf("Student %d: %d\n", i + 1, scores[i]);
    }
}

/* Display statistics — only prints if values are valid (>= 0) */
void displayStatistics(int scores[], int size) {
    if (!validateArrayParams(scores, size)) return;
    printf("\nStatistics:\n");
